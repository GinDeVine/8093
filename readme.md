...
The easiest and least secure method is the four colored boxed between the text area and the image. These represents the red(R), green(G), blue(B), and transparency(A) values of the pixels in which data is to be stored. Due to the few possible combinations, brute forcing this is quite easy.
...
The last and most secure method requires some knowledge of programming. It's written in the 'z' text field and takes a JavaScript function as input 'function(n){ /* SOME CODE HERE */ }' which returns a position in the image (represented as a 1d array) relative to (x,y) to store the n'th path of your message.

Any working JavaScript code within the function can be used, with the requirements that the function z(n) should return an integer (it will be floored) and not the same integer twice for i from 0 to at least four times the length of your message (depending on the RGBA settings). The program will notify you if it thinks your function returns the same value twice or if there won't be enough space in the image to hide your data with the current settings. 

The default function for 'z' is 'function (i){return (i%2)?i:-i;}'.


For easy sharing of these settings, there will appear a string in green text under the text fields in the pattern of [#hidden-with:RGBA#X#Y#Z#]. If this string is added to the large text field when clicking 'hide' or 'seek', the settings in the string will be used. 
